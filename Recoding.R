...
single.to.multiple <- function(r, col){
  ...
  names(test)[scol:ncol(test)] <- paste(col, names(test)[scol:ncol(test)], sep = ".")
  test[is.na(test)] <- 0
  r <- merge(r, test, by="X_uuid", all=T)
  return(r)
}
...
recodingchoices <-function(r) {
  ...
  # df %<>% mutate_if(startsWith(names(df), "waste_disposal"), as.numeric)
  # which(is.na(as.numeric(as.character(df[["waste_disposal"]]))))
  
  r$sewage_visability_recoded %>% table(useNA="always")
  
  response$sector_respondent_agriculture <- ifelse(r$sector_respondent == "agriculture", 1, 0)
...
# 
# 
# topthree <- read.csv("output/summary_sorted_20191214_preliminary_pop_group_aggregated_district_all.csv")
